...
public class CreatureSound : MonoBehaviour {
    ...
    private void addProbabilities()
    {
        ...
        foreach(Sound sound in Sounds)
        {
            ...
            maxProbability += sound.Probability; //Adds up all probabilities of each sound
        }
    }
    ...
    private void PlaySound(int clipNumber, int subSource = -1)
    {
        ...
        else
        {
            if (subSource == 0)
                ...
                targetSource = sound.SubSources[subSource - 1];
        }
    ...
    public void PlaySelectedSoundOnAudioSource(AnimationEvent evnt)
    {
        AudioClip clip = (AudioClip)evnt.objectReferenceParameter;
        int sourceNumber = evnt.intParameter;
        
        int clipNumber = Clips.IndexOf(clip);
        if (clipNumber >= 0) //if it's contained within the preset sounds
            PlaySound(clipNumber, sourceNumber);
    ...
    public void PlaySelectedNoiseSound(AudioClip clip) //usually used for sounds that can cause noise damage
    {
        ...
        this.GetComponent<CreatureMind>().AddJoy();
